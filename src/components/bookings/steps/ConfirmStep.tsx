...
export const ConfirmStep: React.FC<ConfirmStepProps> = ({ data, onSuccess, onBack, language: languageProp }) => {
  ...
  const unitRateSummary = React.useMemo(() => {
    ...
    if (bookingType === 'daily') {
      ...
      const fallbackDaily = Number((unitType as any)?.daily_price || 0) || 0;
      const avg = nights > 0 ? Math.round(total / nights) : fallbackDaily;
      const suffix = nights > 0 ? ` (${nights} ليلة)` : '';
      return { label: 'سعر الوحدة', value: `${avg.toLocaleString()} ر.س / ليلة${suffix}` };
    }

    const { months } =
      data.startDate && data.endDate ? calculateDetailedDuration(data.startDate, data.endDate) : { months: 0 };
    const dailyPrice = Number((unitType as any)?.daily_price || 0) || 0;
    const annualPrice = Number((unitType as any)?.annual_price || 0) || 0;
    const monthlyRate = annualPrice ? Math.round(annualPrice / 12) : dailyPrice * 30;
    ...
    const annualRate = annualPrice ? Math.round(annualPrice) : monthlyRate * 12;
    const suffix = months > 0 ? ` (${months} شهر)` : '';
    ...
    try {
      ...
      if (!invoice) {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          ...
          if (!invoiceError && invoice) createdInvoice = true;
